...
public class MessageOfTheDay extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Watcher.prefix + "hey")) {
            EmbedBuilder hey = new EmbedBuilder();
            hey.setTitle("Hey Buddy! \uD83D\uDE03");
            hey.setDescription(getMessageOfTheDay());
//            Member member = event.getMember();
//            Member shrike = guild.getMemberById(477082166988374017L);
//            String j = event.getMember().getId();
//            Member sh = guild.getMemberById("477082166988374017");
//            String stri = member.getEffectiveName();
//            hey.setFooter(event.getMember().toString());

            hey.setColor(new Color(255, 215, 0));
            hey.setFooter("Created by This Guy", Splotchy.getUser().getAvatarUrl());

            event.getChannel().sendTyping().queue();;
            event.getChannel().sendMessageEmbeds(hey.build()).queue();
            hey.clear();
    ...
    private static String getMessageOfTheDay() {
        ...
        String[] messages = {
                "Conan Exiles was a good game, and Mario should've installed it. Keagan is cracked in Back 4 Blood, watch your back in that shooting range" +
                ". Did I say Conan Exiles was a good game? I did? That isn't anecdotal, it's objective. War Thunder is the largest free-to-play multiplayer online game " +
                "featuring military vehicles from a period spanning from the years before the Second World War until present time. Aviation, ground vehicles, and naval " +
                "forces fight together in one game and even in one battle, just as the real-life battles were fought. - Splotchy",
                ...
                "Expecting life to treat you well because you are a good person is like expec,ting an angry bull not to charge because you are a vegetarian. - Shari R. Barr",
        ...
        return messages[random].toString();
    }
}
